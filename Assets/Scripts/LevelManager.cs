...
namespace Marbleous
{
    ...
    {
        [Header("Main")] [SerializeField] private Mover ballPlayer;
        ...
        [Header("Control")] [SerializeField] private float minShiftForDirection = 20.0f;
        ...
        private void OnDrawGizmos()
        {
            ...
            if (inWorkState)
            {
                for (int i = 0; i < lineList.Length; i++)
                {
                    ...
                    {
                        var setPosition = activeSpline.GetPosition((float) j / activeCountPart) +
        ...
        public float NextObjectIncDirection(Mover ball)
        {
                ...
                if (value.Active)
                {
                    if (value.SplineNumber == ball.SplineNumber)
                    {
                        if (value.SplineRelatePosition > ball.SplineRelatePosition &&
                            value.SplineRelatePosition < findResult)
                        {
            ...
            if (ball != ballPlayer && ballPlayer)
            {
                ...
                {
                    if (ballPlayer.SplineRelatePosition > ball.SplineRelatePosition &&
                        ballPlayer.SplineRelatePosition < findResult)
                    {
